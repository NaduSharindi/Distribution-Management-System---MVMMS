...
j( function() {
   ...
   //alert('hiiiii	3'+altsup);
			...
			//announcement = announcement.replace("&", " and ");
...
var url = "/MMS/updateIntteruption?id="+pid+"&csc=" +encodeURIComponent(csc)+ "&grid="+encodeURIComponent(grid)+"&altsup="+encodeURIComponent(altsup)+"&remarks="+encodeURIComponent(remarks)+"&announcement="+action+"&incharge="+incharge+"&cycle="+encodeURIComponent(cycle)+"&action="+action;
 ...
 var cycle = document.getElementById("cycle").value;
		...
		for (var count = 0; count < len; count++) {
			...
			var url = "/MMS/sendForIntteruption?id="+pid+"&csc=" +encodeURIComponent(csc)+ "&grid="+encodeURIComponent(grid)+"&altsup="+encodeURIComponent(altsup)+"&remarks="+encodeURIComponent(remarks)+"&announcement="+action+"&incharge="+incharge+"&cycle="+encodeURIComponent(cycle)+"&action="+action;
...
function viewAddExcel(){
 ...
 function sendReportEE(){
		...
		.ajax({
			type : 'GET',
			url : "/MMS/sendNotificationIntScheduleEE?cycle=" + cycle,
			data : {},
			contentType : "application/json; charset=utf-8",
			success : function(data) {
				alert("Successfully Done");	
...
<body onload="myMap(new google.maps.LatLng(7.475214, 80.744077), 7);">
									...
									<div class="table-scroll">
									 -->
										<table class="table table-responsive" id="tblProvinces">
											...
											<tbody>
												<form:form method="post" action="createIntSchedule?mode=${province}&type=${type}"
 ...
 <tr>
    <td><button class="btn" type="submit" name="actionButton" class="form-control"><i class="fa fa-hand-o-right  green-bg"></i></button></td>
    	...
    	<td><button id ="btnUpdate" type="button" class="btn btn-secondary" onClick='javascript:saveAll("${model.stringOfLineIds}")'>Update</button></td>
<td><button id ="btnSendForValidation" type="button" class="btn btn-secondary" onClick='javascript:sendForValidationAll("${model.stringOfLineIds}")'>Send for Validation</button></td>
    
    <td><c:if test="${sessionScope.loggedUserRole =='EE' || sessionScope.loggedUserRole =='EEC' || sessionScope.loggedUserRole =='PE'}">
  <button id="pivFormBtn1" class="btn btn-secondary" type="button" onClick="sendReportEE()" >Send Notification</button>
</c:if></td>
<td><c:if test="${sessionScope.loggedUserRole =='PCE' || sessionScope.loggedUserRole =='DGM'}">
 ...
 --%>        							
											         ...
											         <div class="table-scroll">
											           <table class="table table-striped table-bordered table-sm " id="tblAdmin">
														...
														<tbody>
															<c:forEach var="email" items="${model.unReadInspectionReq}" varStatus="status">
																<c:choose>
																<c:when test="${submitType=='Inspection'}">
...
<td style="text-align:left"><textarea rows="3" cols="60" class="form-control" disabled>${email.timeduration}</textarea></td>
 ...
 <td style="text-align:left"><textarea rows="3" cols="60" class="form-control" disabled>${email.rescheduleDate} ${email.fromtime} ${email.totime}</textarea></td>
  --%>
         ...
         <%-- <form:input type="hidden" path="unReadInspectionReq[${status.index}].approvalId"/>
		 --%>														
