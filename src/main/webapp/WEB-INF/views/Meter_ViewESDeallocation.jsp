...
function forwardRequest(id){
	...
	//alert(es);		
		 $.ajax({
			type : 'GET',
			url : "/MMS/deallocateMeterRequestToES/" +id+ "/"+ es + "/",
			data : {},
			contentType : "application/json; charset=utf-8",
			success : function(data) {
				alert("Succesfully De-allocated");
	...
	<th>Attachment</th>
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="email" items="${model.unReadInspectionReq}">
																...
																<c:choose>
																<c:when test="${sessionScope.loggedUserRole=='EE'}">
																	<td><button type="button" onClick='forwardRequest("${email.approvalId}")' title='Recommend & Forward' class="button button2" ><i class="glyphicon glyphicon-share-alt"></i></button> 
																	</td>
																...
																<c:when test="${sessionScope.loggedUserRole=='ES'}">
																	<td><button type ='button' class='button button2' title='' onClick='createTestReport("${email.approvalId}")'><i class="fa fa-check"></i></button>
																</td>
																...
																<td><button type="button" onClick='openPopupSubmitImg1("${email.approvalId}")' class="button button2" ><b>View<b></button> 
																</td>
