...
<script type="text/javascript">
	...
	function getCrew(comid){


		var accountNo = document.getElementById("accNo_" + comid).value;
		alert(accountNo);

		$.ajax({
			type : 'GET',
			url : "/MMS/getMobUserByAccNo/"+accountNo+"/",
			data : {},
			contentType : "application/json; charset=utf-8",
			success : function(response) {
				//alert(response);
				$('#secMobUser_'+comid).empty();
				//Append "None" item
				$('#secMobUser_'+comid).append(
						$('<option>').text("<< Select MNT Crew >>").attr('value',
								""));
				...
				for (var i = 0; i < response.length; i++) {
					var item = response[i];
					$('#secMobUser_'+comid).append(
							$('<option>').text(item.crewName).attr('value',
									item.accessCode));
				}
			}
		});
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="col-lg-9">
								<ol class="breadcrumb">
									<li><a href="#">Home</a></li>
									<li class="active"><span>Job Assign </span></li>
								</ol>
							</div>
								...
								<div class="main-box-body clearfix">
									<div class="table-responsive">
										<table class="table table-responsive" id="tblProvinces">
											...
											<tbody>
												<form:form method="post" action="jobAssignS"
													...
													<table class="table table-responsive" id="tblAdmin">
														...
														<thead>
															<tr>
																...
																<th class="text-center">Mnt Crew</th>
																<th class="text-center"></th>
														...
														<tbody>
															<c:forEach var="line" items="${model.bdDetailList}">
																<tr>
																		<td><input type="text" id="comNO" value="${line.complaintNo}"
																		class="form-control" disabled></td>
																		<td><input type="text" id="accNo_${line.complaintNo}"
																		value="${line.acctNum}" class="form-control" disabled></td>
																		<td><input type="text" id="dateBd_${line.complaintNo}"
																		value="${line.bdDate}" class="form-control" disabled></td>
																		<td><input type="text" id="dateTime_${line.complaintNo}"
																		value="${line.bdTime}" class="form-control" disabled></td>
																		<td><button type='button' class='btn btn-success'
																			onClick='javascript:getCrew("${line.complaintNo}")'>View Available MNT Crew</button></td>
																		<td><form:select id="secMobUser_${line.complaintNo}" path="mobUserObj.accessCode"	onchange="getInfo()">
																		<form:option value="NONE" label="MNT CREW" />
																		<form:options items="${model.mobUser}" />
																		</form:select></td>
																		<td><form:select id="priority_${line.complaintNo}" path="priority"	onchange="getInfo()">
																		...
																		<td><input type="text" id="pcommenta_${line.complaintNo}" value="${line.comments}" class="form-control"></td>
...
<%-- 																							<td><select id="ptype_${line.id}"
																		class="form-control" disabled>
																			...
																			<c:forEach var="linetype" items="${model.linetype}">
																				...
																				<c:choose>
																					<c:when test="${linetype.id != line.lineType}">
																	...
																	<!-- <td><button type='button' class='btn btn-warning'
																			onClick='LoadLinesToEdit("")'>Edit</button></td>
																	 -->
																</tr> 
															...
															<tr>
															<td>
																		<button type="submit" class='btn btn-warning'>Job Assign</button>
																	</td></tr>
														</tbody>
													</table>
													...
													<td>
															<button type='button' class='btn btn-warning'
																onClick='javascript:editAll("${model.stringOfLineIds}")'>View MNT Crew</button>
														</td>
													</tr> --%>
											<%-- 		<tr>
