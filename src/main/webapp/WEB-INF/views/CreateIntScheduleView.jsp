...
function viewReportEE(){
	...
	var url="downloadIntSumReportCCNew?cycle=" +cycle ;
	    var width = 1100;
	    var height = 700;
	    var left = parseInt((screen.availWidth/2) - (width/2));
	    var top = parseInt((screen.availHeight/2) - (height/2));
...
<body onload="myMap(new google.maps.LatLng(7.475214, 80.744077), 7);">
									...
									<div class="table-scroll">
									 -->
										<table class="table table-responsive" id="tblProvinces">
											...
											<tbody>
												<form:form method="post" action="createIntScheduleView?mode=${province}&type=${type}"
 ...
 <%-- <tr>
    <td><button class="btn" type="button" name="actionButton" onclick='drawChartTimeLine("${model.unReadInspectionReq}")' class="form-control">Analyze</button></td>
...
<td style="text-align:center">
																  ...
																  <c:when test="${email.toStatus=='21'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Sent By DEO CC/Pending at Control Center ES ${email.ccApprovalUser}</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='31' }">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Sent By ES CC/Pending at Control Center EE ${email.ccApprovalUser}</textarea>
																  </c:when>
																   <c:when test="${email.toStatus=='41'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Recommended by Control Center EE/Pending at Control Center Planning CE ${email.ccApprovalUser}</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='51'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Recommended by Control Center Planning CE/Pending at Province DGM ${email.ccApprovalUser}</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='61'}">
 ...
 <td style="text-align:left"><textarea rows="3" cols="60" class="form-control" disabled>${email.rescheduleDate} ${email.fromtime} ${email.totime}</textarea></td>
  --%>
																...
																<%-- <td style="text-align:left"> <select id="action_${email.approvalId}" class="form-control"  style="background-color: #94cb71; border-radius: 5px;">
																 ...
																 --%>
																<td style="text-align:left"><textarea rows="3" cols="30" id="intCycle_${email.approvalId}" class="form-control">${email.intCycle}</textarea></td>
...
<button id="pivFormBtn2" class="btn btn-secondary" type="button"  onClick="viewReportExcelEE()" >View Interruption Schedule -Excel</button>
