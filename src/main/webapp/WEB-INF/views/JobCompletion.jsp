...
function removeRequest(id){
    ...
    function viewEstimate(estimateNo,costCenterNo){
    	 var url="downloadEstimateRevise?estimateNo="+estimateNo+"&costCenter="+costCenterNo;
 	    var width = 1100;
 	    var height = 700;
 	    var left = parseInt((screen.availWidth/2) - (width/2));
 	    var top = parseInt((screen.availHeight/2) - (height/2));
 ...
 --%>    <div class="table-responsive">
    <table id= "myTable" class="display table" width="100%">
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="job" items="${model.pcesthmtList}"  varStatus="status">
															
																<tr>
																<td><button id = "saveBtn_${job.id.estimateNo}"  type="button" class="button button3" onClick='saveCompletion("${job.id.estimateNo}")'><i class="glyphicon glyphicon-ok"></i></button></td> 
											                 	<td><button id = "saveBtn_${job.id.estimateNo}"  type="button" class="button button3" onClick='viewEstimate("${job.projectNo}" ,"${job.id.deptId}")'><i class="glyphicon glyphicon-folder-open"></i></button></td> 
											                 	
																<td>${job.id.estimateNo}</td>
																<td>${job.projectNo}</td>
