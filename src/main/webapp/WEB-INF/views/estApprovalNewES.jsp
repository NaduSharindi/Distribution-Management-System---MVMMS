...
function removeRequest(id){
    ...
    function isClickApprove(id,costCenter){
    	...
    	var strUser = e.value;
    	//alert(strUser);
    	if(strUser.includes('NONE')){
    		alert('Please select the responsible EE');
    		event.preventDefault();
			return false;
    	}else{
    	
    		 let r = prompt("You are going to forward the estimate number :" + id+ "\n\nAre you sure?  \n\nPlease enter the comments");
      		 if (r == null || r=="") {
    ...
    function setName(id){
    	...
    	var e = document.getElementById("user_"+id);
    	var strUser = e.value;
    	document.getElementById('ee').value= strUser;
    ...
    function isClickRecommend(id,costCenter){
    	var user = document.getElementById('ee').value;
    ...
    function isClickReject(id,costCenter){
  	  	...
  	  	let r = prompt("You are going to reject the estimate number :" + id+ "\n\nAre you sure?  \n Please enter the reason");
  		if (r == null || r=="") {
...
<div class="upnew">
            	<div class="wrapper">
			<div class = "form">
													<form:form role="form" enctype="multipart/form-data" action="approveEstimateMMSES"
											...
											<form:hidden id = "ee" name ="ee" path = "ee" />
...
<button type="submit" class="button button2" ><b>View<b></button> 
 ...
 --%> 
    ...
    <table style = "font-size: 12px;" id= "myTable" class="display table" width="100%">
        <thead>  
         <tr style="width:2%" class="heading"> 
           ...
           <!--   <th>Select</th>
            ...
            <th><i class="glyphicon glyphicon-circle-arrow-right"></i></th> 
            
         <th><i class="glyphicon glyphicon-folder-open"></i></th>  
          <th><i class="glyphicon glyphicon-print"></i></th>  
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="job" items="${model.listPcesthttModel}"  varStatus="status">
																...
																<td>${job.etimateDt}</td>
																<td>${job.descr}</td>
																<td>${job.eeName}<form:select onchange='setName("${job.estimateNo}")' id = "user_${job.estimateNo}" path ="" >
																	<form:option value="NONE" label="Please select.." />
																	<form:options items="${model.eeList}" />
																</form:select></td>
																
																
																<td><button id = "saveBtn_${job.estimateNo}"  type="submit" class="button button3"  title='Forward for Approval' onclick='isClickApprove("${job.estimateNo}","${job.deptId}")' ><i class="glyphicon glyphicon-circle-arrow-right"></i></button></td> 
																<%-- <td><button id = "saveBtn_${job.estimateNo}"  type="submit" class="button button3"  title='Upload Sketch' onclick='myFunction()' ><i class="glyphicon glyphicon-circle-arrow-right"></i></button></td> 
																 --%>
    ...
    <div class="table-responsive">
    <table style = "font-size: 12px;" id= "myTable1" class="display table" width="100%">
    <tr>
    <td>Upload file</td>
