    ...
    <table style = "font-size: 12px;" id= "myTable" class="display table" width="100%">
        <thead>  
         <tr style="width:2%" class="heading"> 
           ...
           <!--   <th>Select</th>
            ...
            <c:if test="${sessionScope.loggedUserRole=='EE' || sessionScope.loggedUserRole=='CE' || sessionScope.loggedUserRole=='DGM' || sessionScope.loggedUserRole=='AGM'}">
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="job" items="${model.listPcesthttModel}"  varStatus="status">
																...
																<td><form:input id = "saveBtn_${job.estimateNo}"  type="submit" class="button button4" path ="approve" value="Approve" onclick='isClickApprove("${job.estimateNo}","${job.deptId}","${job.projectNo}")' ></form:input></td> 
											                 	
																</c:if>
																<c:if test="${job.normDefault==1}">
																<td><form:input id = "saveBtn_${job.estimateNo}"  type="submit" class="button button4" path ="recommend" value="Recommend" onclick='isClickRecommend("${job.estimateNo}","${job.deptId}","${job.projectNo}")'></form:input></td> 
																...
																<%-- <td> <form:input path ="rejectReason" type="text" class="form-control" style="width: 100%;" id="txtReason" placeholder="Enter reject reason here.." title="Type remarks"></form:input>
											                 	...
											                 	<td><form:input id = "saveBtn_${job.estimateNo}"  type="submit" class="button button3" path ="reject"  value ="Reject" onclick='isClickReject("${job.estimateNo}","${job.deptId}","${job.projectNo}")'></form:input></td> 
											                 	</c:if>
											                 	 <c:if test="${sessionScope.loggedUserRole=='ES'}">
											                 	 <td><form:input id = "saveBtn_${job.estimateNo}"  type="submit" class="button button4" path ="sendforvalidation" value="Forward" onclick='isClickSendForValication("${job.estimateNo}","${job.deptId}","${job.projectNo}")'></form:input></td> 
