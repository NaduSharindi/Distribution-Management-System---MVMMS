...
j( function() {
	...
	function saveAll(stringIDs) {
		...
		for (var count = 0; count < len; count++) {
			...
			var url = "/MMS/updateIntteruption?id="+pid+"&csc=" +csc+ "&grid="+grid+"&altsup="+altsup+"&remarks="+remarks+"&announcement="+announcement+"&incharge="+incharge+"&cycle="+cycle+"&action="+action;
 ...
 var cycle = document.getElementById("cycle").value;
		...
		for (var count = 0; count < len; count++) {
			...
			var url = "/MMS/sendForIntteruption?id="+pid+"&csc=" +csc+ "&grid="+grid+"&altsup="+altsup+"&remarks="+remarks+"&announcement="+action+"&incharge="+incharge+"&cycle="+cycle+"&action="+action;
...
function viewReportEE(){
	...
	var url="downloadIntSumReportCCNewPhm?cycle=" +cycle ;
	    var width = 1100;
	    var height = 700;
	    var left = parseInt((screen.availWidth/2) - (width/2));
	    var top = parseInt((screen.availHeight/2) - (height/2));
...
function viewReportExcel(){
	...
	var cycle = document.getElementById("cycle").value;
	//window.location.href='downloadIntSumReportCCNewPhmExcel?cycle=${email.filename1}'"
	
	var url="downloadIntSumReportCCNewPhmExcel?cycle=" +cycle ;
...
function viewAddExcel(){
 ...
 function sendReport(){
	...
	.ajax({
		type : 'GET',
		url : "/MMS/sendNotificationIntSchedule?cycle=" + cycle,
		data : {},
		contentType : "application/json; charset=utf-8",
		success : function(data) {
...
<body onload="myMap(new google.maps.LatLng(7.475214, 80.744077), 7);">
									...
									<div class="table-scroll">
									 -->
										<table class="table table-responsive" id="tblProvinces">
											...
											<tbody>
												<form:form method="post" action="createIntScheduleRecommended?mode=${province}&type=${type}"
 ...
 <tr>
  ...
  <tr>
   ...
   --%>
 <tr><td >Cycle</td>
 <td><form:select class="form-control" style="background-color: #FFFFF; border-radius: 5px;" id="cycle" path="cycle" ><form:options items="${model.cycleList}" /></form:select></td>
 </tr>
...
<%--  <tr>
 ...
 <tr>
    <td><button class="btn" type="submit" name="actionButton" class="form-control"><i class="fa fa-hand-o-right  green-bg"></i></button></td>
     <td><button id="pivFormBtn1" class="btn btn-secondary" type="button" onClick="viewReportEE()" >View Interruption Schedule</button></td>
    <td><button id="pivFormBtn2" class="btn btn-secondary" type="button"  onClick="viewReportExcel()" >View Interruption Schedule -Excel</button></td>
											         ...
											         <div class="table-scroll">
											           <table class="table table-striped table-bordered table-sm " id="tblAdmin">
														...
														<c:when test="${submitType=='Interruption'}">
															...
															<!-- <th class="text-center" class="form-control">Status</th>
														   ...
														   <th class="text-center" class="form-control">Section to be interrupted</th>
															...
															<!--  <th class="text-center" class="form-control">Action</th>
															 -->  <th class="text-center" class="form-control">Interruption Cycle</th>
...
<c:when test="${submitType=='Interruption'}">
																...
																<tr>
																<th style="text-align:center;background-color:#23cc45"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																...
																<c:when test="${email.toStatus=='96'}">
																
																<th style="text-align:center;background-color:#235ecc"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																
																<c:when test="${email.toStatus=='95'}">
																
																<th style="text-align:center;background-color:#d1cb24"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																
																<c:when test="${email.toStatus=='97'}">
																
																<th style="text-align:center;background-color:#d44b22"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																
																<c:when test="${email.toStatus=='21'}">
																
																<th style="text-align:center;background-color:#cf23bd"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																<c:when test="${email.toStatus=='31'}">
																
																<th style="text-align:center;background-color:#cf23bd"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																<c:when test="${email.toStatus=='41'}">
																
																<th style="text-align:center;background-color:#cf23bd"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																<c:when test="${email.toStatus=='51'}">
																
																<th style="text-align:center;background-color:#cf23bd"><input style="text-align:center" type="text" id="" value="${email.approvalId}"  class="form-control" disabled></th>
																</c:when>
																
																<c:when test="${email.toStatus=='61'}">
...
<%-- <td style="text-align:center">
																  ...
																  <c:when test="${email.toStatus=='21'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Sent By DEO CC/Pending at Control Center ES</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='31' }">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Sent By ES CC/Pending at Control Center EE</textarea>
																  </c:when>
																   <c:when test="${email.toStatus=='41'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Recommended by Control Center EE/Pending at Control Center Planning CE</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='51'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Recommended by Control Center Planning CE/Pending at Province DGM</textarea>
																  </c:when>
																  <c:when test="${email.toStatus=='61'}">
																  <textarea style="text-align:left" id="" class="form-control" disabled>Approved by Province DGM</textarea>
																  </c:when>
																  ...
																  <c:when test="${email.toStatus=='80'}">
																  <input style="text-align:left" type="text" id="" value="Rescheduled" class="form-control" disabled>
																  </c:when>
																  
																   <c:when test="${email.toStatus=='77'}">
																  <input style="text-align:left" type="text" id="" value="Cancelled at Control Center" class="form-control" disabled>
																  </c:when>
																 
																  <c:when test="${email.toStatus=='96'}">
																  <input style="text-align:left" type="text" id="" value="Recommended" class="form-control" disabled>
																  </c:when>
...
<td style="text-align:left"><textarea rows="3" cols="60" class="form-control" disabled>${email.timeduration}</textarea></td>
 ...
 <td style="text-align:left"><textarea rows="3" cols="60" class="form-control" disabled>${email.rescheduleDate} ${email.fromtime} ${email.totime}</textarea></td>
  --%>
																...
																<td style="text-align:left"><textarea rows="3" cols="20" id="csc_${email.approvalId}" class="form-control" disabled>${email.csc}</textarea></td>
																<td style="text-align:left"><textarea rows="3" cols="20" id="grid_${email.approvalId}" class="form-control" disabled>${email.grid}</textarea></td>
																
																<td style="text-align:left"><textarea rows="3" cols="30" id="altsup_${email.approvalId}" class="form-control" disabled>${email.altSupArrangement}</textarea></td>
																
																<td style="text-align:left"><textarea rows="3" cols="30" id="remarks_${email.approvalId}" class="form-control" disabled>${email.remarks}</textarea></td>
																...
																<%-- <td style="text-align:left"> <select id="action_${email.approvalId}" class="form-control"  style="background-color: #94cb71; border-radius: 5px;">
																 ...
																 --%>
																<td style="text-align:left"><textarea rows="3" cols="30" id="intCycle_${email.approvalId}" class="form-control" disabled>${email.intCycle}</textarea></td>
																
																
																<%-- <td><form:select class="form-control" style="background-color: #FFFFF; border-radius: 5px;" id="cycle" path="email.remarks" ><form:options items="${model.cycleList}" /></form:select></td>
  --%>
...
<button id="pivFormBtn1" class="btn btn-secondary" type="button" onClick="viewReport()" >View Interruption Schedule</button>
<button id="pivFormBtn2" class="btn btn-secondary" type="button"  onClick="viewReportExcel()" >View Interruption Schedule -Excel</button>
<button id="pivFormBtn3" class="btn btn-secondary" type="button"  onClick="viewAddExcel()" >View Interruption Advertisement -Excel</button>
 ...
 --%>
 <c:if test="${sessionScope.loggedUserRole =='EE' || sessionScope.loggedUserRole =='EEC' || sessionScope.loggedUserRole =='PE' || sessionScope.loggedUserRole =='PCE' || sessionScope.loggedUserRole =='DGM'}">
 <!--  <button id="pivFormBtn1" class="btn btn-secondary" type="button" onClick="sendReport()" >Send Notification</button>
  --> 
 <!-- <button id="pivFormBtn1" class="btn btn-secondary" type="button" onClick="viewReportEE()" >View Interruption Schedule</button>
<button id="pivFormBtn2" class="btn btn-secondary" type="button"  onClick="viewReportExcel()" >View Interruption Schedule -Excel</button>
<button id="pivFormBtn3" class="btn btn-secondary" type="button"  onClick="viewAddExcelEE()" >View Interruption Advertisement -Excel</button>
  --></c:if>
