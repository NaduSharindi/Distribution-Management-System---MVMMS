...
function removeRequest(id){
    ...
    function saveCompletion(id){
    	...
    	$.ajax({
			type : 'GET',
			url : "/MMS/completionEst?id=" +id +"&complesion="+complesion+"&remarks="+encodeURIComponent(remarks),
			data : {},
			contentType : "application/json; charset=utf-8",
			success : function(data) {
 ...
 --%>    <div class="table-responsive">
    <table id= "myTable" class="display table" width="100%">
        <thead>  
          ...
          <!--  <th></th>
			 -->
																		...
																		<th>Labour Hours</th>
																		<th>Labour Cost</th>
																		<th>Number of days for the inspection</th>
																		<th>Subsistance</th>
																		<th>Total Cost</th>
																		<th>Temporary Site Cost </th>
																		<th>Total Line Length </th>
																		<th>Total Tower </th>
																		<th>Transport Cost </th>
																		<th>Transport Manual </th>
																		<th>Hot Line Allowances</th>
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="pcbModel" items="${SaveSample.insList}"  varStatus="status">
															
																<tr>
																<td><button id = "saveBtn_${pcbModel.inspectionId}"  type="button" class="button button3" onClick='saveCompletion("${pcbModel.inspectionId}")'><i class="glyphicon glyphicon-floppy-disk"></i></button></td> 
											                 	<td><button id = "saveBtn_${pcbModel.inspectionId}"  type="button" class="button button3" onClick='viewEstimate("${pcbModel.westimateNo}" ,"${pcbModel.phmBranch}")'><i class="glyphicon glyphicon-folder-open"></i></button></td> 
											                 	
																<%-- <td>${status.count}</td>
																 --%><td>${pcbModel.inspectionId}</td>
          														<td>${pcbModel.westimateNo}</td>
          														<td><textarea id="remarks_${pcbModel.inspectionId}" class="form-control">${pcbModel.remarks}</textarea></td>
											                 	<c:if test="${sessionScope.loggedUserRole=='ES'}">
			
											                 	<td><select id="completion_${pcbModel.inspectionId}"
																	onchange="" >
 																...
 																<td><select id="completion_${pcbModel.inspectionId}"
																	onchange="" >
																	<c:choose>
                                                                                    <c:when test="${pcbModel.completion=='1'}">
                                                                                        <option value="${pcbModel.completion}"
                                                                                            selected="selected">Completed</option>
                                                                                             <option value="2">Partially Completed</option>
                                                                                             <option value="3">Not Completed</option>
											                			
                                                                                    </c:when>

                                                                                    <c:when test="${pcbModel.completion=='2'}">
                                                                                        <option value="${pcbModel.completion}"
                                                                                            selected="selected">Partially Completed</option>
                                                                                             <option value="1">Completed</option>
                                                                                             <option value="3">Not Completed</option>
											                			
                                                                                    </c:when>

                                                                                    <c:when test="${pcbModel.completion=='3'}">
                                                                                          <option value="${pcbModel.completion}"
                                                                                            selected="selected">Not Completed</option>
                                                                                             <option value="1">Completed</option>
                                                                                             <option value="2">Partially Completed</option>
											                								</c:when>
																...
																<td>${pcbModel.description}</td>
																<td>${pcbModel.cycle}</td>
																<td><c:choose>
                                                                                    <c:when test="${pcbModel.status=='75'}">
                                                                                        Modify
                                                                                    </c:when>
									...
									<td>${pcbModel.labourHoursForEst}</td>
									<td>${pcbModel.labourValueForEst}</td>
									<td>${pcbModel.noDaysForTheIns}</td>
									<td>${pcbModel.subsistance}</td>
									<td>${pcbModel.tatalCost}</td>
									<td>${pcbModel.temporarySiteCost}</td>
									<td>${pcbModel.totalLineLength}</td>
									
									<td>${pcbModel.totalNoTowers}</td>
									
									<td>${pcbModel.transport}</td>
									
									<td>${pcbModel.transportManual}</td>
									
									<td>${pcbModel.hotLineAllowances}</td>
