...
function removeRequest(id){
    ...
    function viewEstimate(estimateNo){
    	var url="viewGantry?id="+estimateNoGloble;
  	     var width = 1100;
 	    var height = 700;
 	    var left = parseInt((screen.availWidth/2) - (width/2));
 	    var top = parseInt((screen.availHeight/2) - (height/2));
    ...
    function isClickForward(id){

		
		let r = prompt("You are going to forward the gatry details for the validation. \n\nAre you sure?  \n\nPlease enter the comments");
 		 if (r == null || r=="") {
			...
			return false;
 		  	...
 		  	document.getElementById('gantryid').value= id;
...
<div class="upnew">
            	<div class="wrapper">
			<div class = "form">
													<form:form role="form" action="displayAllGantry"
											method="post" modelAttribute="model">
											<form:hidden id ="gantryid" name = "gantryid" path="gantryid" />
 ...
 --%>    <div class="table-responsive">
    <table style = "font-size: 12px;" id= "myTable" class="display table" width="100%">
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="job" items="${model.gantryListEdit}"  varStatus="status">
																...
																<td>${job.code}</td>
																<td>${job.name}</td>
																 <td><button id = "saveBtn_${job.id}"  type="button" class="button button3" title='View Gantry Details' onClick='viewGantry("${job.id}")'><i class="glyphicon glyphicon-folder-open"></i></button></td> 
											                 	<td><button id = "saveBtn_${job.id}"  type="submit" class="button button3"  onclick='isClickForward("${job.id}")' ><i class="glyphicon glyphicon-share-alt"></i></button></td> 
	 ...
	 <td><form:input id = "saveBtn_${job.estimateNo}"  type="submit" class="button button4" title='Forward to next level' path ="forward" value="Forward" onclick='isClickForward("${job.estimateNo}","${job.deptId}")'></form:input></td> 
																...
																<td><button id = "saveBtn_${job.estimateNo}"  type="button" class="button button3" title='View Estimate' onClick='viewEstimate("${job.estimateNo}" ,"${job.deptId}")'><i class="glyphicon glyphicon-share-alt"></i></button></td> 
