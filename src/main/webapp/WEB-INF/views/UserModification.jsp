...
function removeRequest(id){
    ...
    function approve(estimateNo,costCenter,approveorrecommend){
  	  
  	  
  		var r = confirm("You are going to approve the estimate number :" + estimateNo+ "\n\nAre you sure?");
  		var urlNew ="/MMS/approveEstimate?estimateNo="+ estimateNo +"&deptId="+costCenter;
  		
  		
  		if(approveorrecommend == 0){
  			urlNew = "/MMS/approveEstimate?estimateNo="+ estimateNo +"&deptId="+costCenter;
  		}else{
  			urlNew = "/MMS/recommendEstimate?estimateNo="+ estimateNo +"&deptId="+costCenter;
  		...
  		if (r == true) {
  			$.ajax({
  				...
  				success : function(result) {
  					//document.getElementById("spanlable").value = result;
  					alert(result + ',if the page is not refreshed, please press the Refresh button');
  					window.location.reload();
  				},
  	  ...
  	  if (!empt)
  		...
  		return false;
  			...
  			if (r == true) {
  				$.ajax({
  					type : 'GET',
  					url : "/MMS/rejectEstimate?estimateNo="+ estimateNo +"&deptId="+CostCenter+"&reason="+empt,
  					data : {},
  					contentType : "application/json; charset=utf-8",
  					success : function(result) {
  						alert(result);
    ...
    function userRegisterTest(userid){
    	//alert('hii');
   	 var url="modifyUserRegistration?userid="+userid;
   	var width = 1100;
	    var height = 700;
	    var left = parseInt((screen.availWidth/2) - (width/2));
       var top = parseInt((screen.availHeight/2) - (height/2));
...
<div class="upnew">
            	<div class="wrapper">
			<div class = "form">
													<form:form role="form" action="approveEstimateMMS"
 ...
 --%>    <div class="table-responsive">
    <table id= "myTable" class="display table" width="100%">
        ...
        <c:set var="count" value="0" scope="page" />
															...
															<c:forEach var="job" items="${model.userList}"  varStatus="status">
																...
																<td><button  type="button" class="button button3" onClick='userRegisterTest("${job.userId}")'>Modify</button></td> 
